...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $password, $email, $additional_data = [], $get_last_name=FALSE, $groups = [])
	{		
		...
		$data = [
			...
			'username' => strtolower($additional_data['first_name'].'_'.$additional_data['father_name'].'_'.$get_last_name),
			'password' => $password,
			'email' => $email,
		...
		if (empty($groups)) {
			...
			$invoice_date = [
				'user_id' => $id,
				'zone_id' => $additional_data['zone_id'],
				'surname' => $additional_data['last_name'],
				'fy_'.$year.'_amt_to_collect' => $resultdata->amount,
				'fy_'.$year.'_balance_count_amt' => 1.00,
				'fy_'.$year.'_balance_amt' => $resultdata->amount,
			];

			$this->db->insert('invoices', $invoice_date);
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, phone, id, password, active, last_login')
						  ->where($this->identity_column, $identity)
	...
	{		
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			$data = ['ip_address' => '', 'login' => $identity, 'time' => date("Y-m-d h:m:s",time())];
			if ($this->config->item('track_login_ip_address', 'ion_auth'))
			{
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select([
			    ...
			    $this->tables['zone_master'].'.zone_name',
			    $this->tables['surname_master'].'.surname',
			    $this->tables['groups'].'.description as group_name',			    
			    $this->tables['users_groups'].'.group_id'
			]);
	...
	public function update_group_id($user_id = FALSE, $group_id = FALSE)
	{	
		
		$this->db->where('user_id',$user_id);
		$this->db->update('users_groups', ['group_id' => $group_id]);
